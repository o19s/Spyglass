...
Sg.SolrSearcher = Sg.Searcher.extend({
  ...
  getFacets: function() {
    ...
    var formatDateFacets = function(fieldName, rawFacetValues) {
      ...
      for (var rawFacetValue in rawFacetValues) {
        ...
        year = Sg.Utils.formatISODate(rawFacetValue, "year");
  ...
  addFacet: function (field, value, redirect) {
    ...
    if (redirect) {
      self.params.start = 0;
      var urlHash = '/search/' + Sg.Utils.returnSearchHash(self.params.q, this.getSelectedFacets(), self.params.sort);
  ...
  removeSelectedFacet: function (facet) {
    ...
    var urlHash = '/search/' + Sg.Utils.returnSearchHash(params.q, this.getSelectedFacets(), params.sort);
    window.location.hash = urlHash;
  }
});